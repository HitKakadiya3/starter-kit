...
import { useCustomerThankyou, type CategoryResult } from '@/hooks/useCustomerThankyou';
...
import { initialScores, type Scores, type TraitPercentage } from '@/utils/scoring';
...
/**
 * Tagged union describing where the rendered report's data comes from.
 * `client` powers the dev preview path (existing `state.scores` flow).
 * `api` powers the live thankyou-driven render added in Module 5.
 */
type ReportSource =
  | { kind: 'client'; scores: Scores; careerPurchased?: boolean }
  | {
      kind: 'api';
      // The encrypted qid from the URL — propagated so the "View career
      // report" button can navigate to `/career-report?qid=...` and the
      // page can be opened in a fresh browser without prior session state.
      qid: string | null;
      type: string;
      identity: 'A' | 'T';
      ...
      stressMarker: number;
      traitPercentages: TraitPercentage[] | null;
      careerPurchased?: boolean;
    };
...
const AXIS_LABEL_MAP: Array<{
  dim: TraitPercentage['dim'];
  leftLabel: string;
  rightLabel: string;
  trait: keyof MbtiResult['traits'];
  firstLetter: string;
  typeIndex: number;
}> = [
  { dim: 'E/I', leftLabel: 'Extraversion', rightLabel: 'Introversion', trait: 'EI', firstLetter: 'E', typeIndex: 0 },
  { dim: 'S/N', leftLabel: 'Sensing',      rightLabel: 'Intuition',    trait: 'SN', firstLetter: 'S', typeIndex: 1 },
  { dim: 'T/F', leftLabel: 'Thinking',     rightLabel: 'Feeling',      trait: 'TF', firstLetter: 'T', typeIndex: 2 },
  { dim: 'J/P', leftLabel: 'Judging',      rightLabel: 'Perceiving',   trait: 'JP', firstLetter: 'J', typeIndex: 3 },
];

/**
 * Adapt the persisted `MbtiResult` into the `TraitPercentage[]` shape that
 * `<HeroSection>` expects. Returns `null` when the persisted result is
 * unavailable so the caller can fall back to a type-only synthesis.
 */
function adaptMbtiResultToTraitPercentages(result: MbtiResult | null): TraitPercentage[] | null {
  if (!result) return null;
  return AXIS_LABEL_MAP.map(({ dim, leftLabel, rightLabel, trait, firstLetter }) => {
    const t = result.traits[trait];
    const dominantLabel = t.letter === firstLetter ? leftLabel : rightLabel;
    return {
      dim,
      leftLabel,
      rightLabel,
...
/**
 * Map the backend's `category_result` array onto the `TraitPercentage[]`
 * shape `<HeroSection>` expects. The API gives `sixteentypes_full_result.percent`
 * as the percentage of the *first* pole named in `category` (e.g. for
 * `extraversion_introversion` it is `% Extraversion`), so the dominant
 * pole's percentage is `100 - percent` when the dominant `letter` is the
 * second pole. Output uses the `[50, 100]` dominant-percent convention
 * `<HeroSection>`'s bar geometry expects.
 *
 * Returns `null` when the array is missing, malformed, or doesn't include
 * all four MBTI axes — callers fall back to the type-only synthesis.
 */
const CATEGORY_AXIS_MAP: Record<
  string,
  { dim: TraitPercentage['dim']; leftLabel: string; rightLabel: string; firstLetter: string }
> = {
  extraversion_introversion: { dim: 'E/I', leftLabel: 'Extraversion', rightLabel: 'Introversion', firstLetter: 'E' },
  sensing_intuition:         { dim: 'S/N', leftLabel: 'Sensing',      rightLabel: 'Intuition',    firstLetter: 'S' },
  thinking_feeling:          { dim: 'T/F', leftLabel: 'Thinking',     rightLabel: 'Feeling',      firstLetter: 'T' },
  judging_perceiving:        { dim: 'J/P', leftLabel: 'Judging',      rightLabel: 'Perceiving',   firstLetter: 'J' },
};

function mapCategoryResultToTraitPercentages(
  categories: CategoryResult[] | undefined,
): TraitPercentage[] | null {
  if (!Array.isArray(categories) || categories.length === 0) return null;
  const byCategory = new Map(categories.map((c) => [c.category, c]));
  const out: TraitPercentage[] = [];
  for (const [key, axis] of Object.entries(CATEGORY_AXIS_MAP)) {
    const cat = byCategory.get(key);
    const full = cat?.sixteentypes_full_result;
    if (!cat || !full || typeof full.percent !== 'number' || typeof full.letter !== 'string') {
      return null;
    }
    const dominantIsFirst = full.letter === axis.firstLetter;
    // Clamp to [50, 100] so the bar geometry never inverts on edge values.
    const dominantPercent = Math.max(
      50,
      Math.min(100, Math.round(dominantIsFirst ? full.percent : 100 - full.percent)),
    );
    out.push({
      dim: axis.dim,
      leftLabel: axis.leftLabel,
      rightLabel: axis.rightLabel,
      dominantLabel: dominantIsFirst ? axis.leftLabel : axis.rightLabel,
      percentage: dominantPercent,
    });
  }
  return out;
}
...
/**
 * Type-only fallback when neither `category_result` nor a persisted
 * `MbtiResult` is available. Renders the four hero bars at 50% with the
 * dominant label derived from each letter of the type code (e.g. ESFP →
 * Extraversion, Sensing, Feeling, Perceiving). Without this the api path
 * would render a bar-less hero on responses that omit `category_result`.
 */
function buildTraitPercentagesFromType(type: string): TraitPercentage[] {
  return AXIS_LABEL_MAP.map(({ dim, leftLabel, rightLabel, firstLetter, typeIndex }) => ({
    dim,
    leftLabel,
    rightLabel,
    dominantLabel: type[typeIndex] === firstLetter ? leftLabel : rightLabel,
    percentage: 50,
  }));
}
...
async function downloadReportPdf(args: {
  ...
  // Pre-fetch every famous-person avatar in parallel so react-pdf can embed
  // them as data URLs without doing network I/O during render. Names without
  // a mapped PNG are skipped — the PDF falls back to coloured initials.
  //
  // The avatar map (famousAvatars.ts) is keyed by English names, but the JA
  // dataset stores katakana/kanji names ("トーマス・エジソン"), so a direct
  // lookup misses every entry in JP reports. We fetch the EN dataset for the
  // same type and pair entries by index — the EN and JA famousPeople arrays
  // are kept in lockstep — to recover the English avatar key, then store the
  // result under the *JA* name so the PDF (which renders `p.name` from
  // `args.data`) finds it.
  const enData = args.locale === 'ja' ? getPremiumTypeDataLocalized(args.type, 'en') : null;
  const famousAvatarEntries = await Promise.all(
    args.data.famousPeople.map(async (p, i) => {
      const lookupName = enData?.famousPeople[i]?.name ?? p.name;
      const url = getFamousAvatar(lookupName);
      if (!url) return null;
      const dataUrl = await fetchAsDataUrl(url);
      return dataUrl ? ([p.name, dataUrl] as const) : null;
    })
  );
  const famousAvatars = Object.fromEntries(
    famousAvatarEntries.filter((e): e is readonly [string, string] => e !== null)
  );
...
const PremiumReportPage = () => {
  ...
  if (scores) return <ReportContent source={{ kind: 'client', scores, careerPurchased }} />;
...
const ReportGuardedPlaceholder = ({ qid }: { qid: string | null }) => {
  ...
  return <ReportLiveLoader qid={qid} />;
};

/**
 * Mount-time loader that drives the live report from `customer/thankyou`.
 * Renders a full-screen spinner while the call is in flight, an error
 * variant on failure, and `<ReportContent kind="api">` on success.
 */
const ReportLiveLoader = ({ qid }: { qid: string | null }) => {
  const { t } = useTranslation();
  const navigate = useLocalizedNavigate();
  const { data, loading, error } = useCustomerThankyou();

  // The customer/thankyou response carries `redirect_page` (mirrors the funnel
  // enum). The resume guard skips the /questions/results validation for
  // /results because that endpoint only accepts the raw integer qid and the
  // URL here carries the encrypted form, so this is the only place the page
  // can detect that the backend believes the user still belongs on
  // /checkout / /cross-sell / /details. Apply the same cascading override as
  // DetailsPage so client-side `crossSellResolved` / `customerUpdateSubmitted`
  // flags trump a stale backend echo, then bounce. `qidParamForRoute`
  // switches the URL qid back to the raw integer for funnel routes — the
  // pages they target call /questions/results which rejects the encrypted id.
  const redirectRoute = useMemo<string | null>(() => {
    if (!data?.redirect_page) return null;
    const effective = resolveEffectiveRedirect(data.redirect_page, getSession());
    const route = resolveRedirect(effective);
    return route === '/results' ? null : route;
  }, [data]);

  useEffect(() => {
    if (!data || !redirectRoute) return;
    const qidParam = qidParamForRoute(
      redirectRoute,
      data.quiz_result_id,
      data.encrypted_quiz_result_id,
    );
    navigate(withPromoParams(`${redirectRoute}?qid=${qidParam}`), { replace: true });
  }, [data, redirectRoute, navigate]);

  // Hold the spinner while the redirect navigation flushes — otherwise the
  // report would render a frame before unmount.
  ...
  if (error) {
    return <ReportErrorState qid={qid} />;
  }

  if (!data) {
    // Defensive: the loading/error gates above should have covered this.
    return null;
  }

  // Validate the personality type against the canonical 16-type allow-list
  // before passing it downstream — `getPremiumTypeData` would throw on any
  // non-canonical value (R3 in plan).
  ...
  // Prefer the backend's per-axis breakdown (real test percentages); fall
  // back to a persisted client-side `MbtiResult` (dev tools / pre-API
  // sessions); finally synthesise from the type code so bars always render.
  const traitPercentages =
    mapCategoryResultToTraitPercentages(detail.category_result) ??
    adaptMbtiResultToTraitPercentages(readResult()) ??
    buildTraitPercentagesFromType(detail.personality_type);
  const source: ReportSource = {
    kind: 'api',
    qid,
    type: detail.personality_type,
    identity: detail.identity,
    turbulentPercent: detail.turbulent_percent,
    socialBattery: detail.social_battery,
    stressMarker: detail.stress_marker,
    traitPercentages,
    careerPurchased: data.is_cross_sale_transaction_completed ?? false,
  };
  return <ReportContent source={source} />;
};

/**
 * Inline error variant rendered when the thankyou call fails or the response
 * is structurally invalid. Surfaces the qid for support and offers a retry +
 * a back-to-home action.
 */
const ReportErrorState = ({ qid }: { qid: string | null }) => {
  const { t } = useTranslation();
  const navigate = useLocalizedNavigate();
  return (
...
const ReportContent = ({ source }: { source: ReportSource }) => {
  const { t } = useTranslation();
  const navigate = useLocalizedNavigate();
  const careerPurchased = source.careerPurchased ?? false;

  // Single navigation entry-point for "View career report" — keeps the
  // dev-preview (state.scores) and live (qid in URL) paths aligned across
  // the desktop sidebar, the inline banner, and the mobile banner.
  const goToCareerReport = () => {
    if (source.kind === 'client') {
      navigate('/career-report', { state: { scores: source.scores } });
      return;
    }
    navigate(
      source.qid
        ? `/career-report?qid=${encodeURIComponent(source.qid)}`
        : '/career-report',
    );
  };

  // `useResults` must be called unconditionally to satisfy React's rules of
  // hooks. On the api path, `initialScores` produces a discarded zero-result
  // — the api branch consumes `source.type` / `source.traitPercentages`
  // directly. The internal `useMemo` chain inside `useResults` is cheap.
  const clientResults = useResults(source.kind === 'client' ? source.scores : initialScores);
  const type = source.kind === 'client' ? clientResults.type : source.type;
  // Memoize so the empty-array fallback on the api path keeps a stable
  // reference across renders — otherwise the useEffect below sees a new
  // dependency every render and reattaches the download listener.
  const traitPercentages = useMemo<TraitPercentage[]>(
    () =>
      source.kind === 'client'
        ? clientResults.traitPercentages
        : (source.traitPercentages ?? []),
    [source, clientResults.traitPercentages],
  );

  const stressMarkerOverride = source.kind === 'api' ? source.stressMarker : undefined;
  const socialBatteryOverride = source.kind === 'api' ? source.socialBattery : undefined;
  ...
  useEffect(() => {
    const onDownload = () => {
      ...
      const downloadTurbulentPercent =
        source.kind === 'api' ? source.turbulentPercent : result?.turbulentPercent;
      void downloadReportPdf({
        data,
        type,
        traitPercentages,
        avatarUrl,
        turbulentPercent: downloadTurbulentPercent,
  ...
  }, [data, type, traitPercentages, result, source, locale]);
  ...
  return (
    ...
    >
      ...
      <div className="lg:flex max-w-[1280px] mx-auto">
        <Sidebar
          careerPurchased={careerPurchased}
          onViewCareer={goToCareerReport}
        />
        <main id="report-content" className="flex-1 min-w-0">
          ...
          <StressSection data={data} type={type} result={result} markerPositionOverride={stressMarkerOverride} />
          <RelationshipsSection data={data} />
          <FriendshipsSection data={data} type={type} result={result} socialBatteryOverride={socialBatteryOverride} />
        ...
        <div className="grid md:grid-cols-[1fr,auto] gap-8 items-center">
          <div>
            ...
            <div className="flex items-center gap-3 mb-4">
              <span className="inline-flex items-center py-2 pl-4 pr-[calc(1rem+0.1em)] rounded-full bg-white/20 backdrop-blur text-3xl md:text-5xl font-extrabold tracking-widest">
                {type}
              </span>
            </div>
...
const StressSection = ({ data, type, result, markerPositionOverride }: {
  data: PremiumTypeData;
  type: string;
  result: MbtiResult | null;
  markerPositionOverride?: number;
}) => {
  const stressBaseline = stressBaselineByType[type] ?? 50;
  // When an override is provided (api path), use it directly; otherwise blend
  // the per-type baseline with the persisted turbulentPercent (dev path).
  const markerPosition =
    markerPositionOverride !== undefined
      ? clamp(Math.round(markerPositionOverride), 2, 98)
      : clamp(
          Math.round(
...
const FriendshipsSection = ({ data, type, result, socialBatteryOverride }: {
  data: PremiumTypeData;
  type: string;
  result: MbtiResult | null;
  socialBatteryOverride?: number;
}) => {
  ...
  // When an override is provided (api path), use it directly; otherwise blend
  // the per-type baseline with the persisted EI directional percent (dev path).
  const socialBattery =
    socialBatteryOverride !== undefined
      ? clamp(Math.round(socialBatteryOverride), 0, 100)
      : clamp(
          Math.round(
    ...
    <Section id="friendships">
      ...
      <div className="bg-card border border-border rounded-2xl p-6 mb-8">
        <div className="flex items-center justify-between mb-3">
          ...
          <span data-testid="social-battery-percent" className="text-2xl font-bold" style={{ color: 'var(--report-accent)' }}>{socialBattery}%</span>
        </div>
...
const FamousSection = ({ data }: { data: PremiumTypeData }) => {
  const { t } = useTranslation();
  const locale = useLocale();
  // famousAvatars.ts is keyed by English names. The JA dataset replaces those
  // with katakana/kanji, so direct lookups miss. Pull the EN dataset for the
  // same type and pair entries by index — the EN/JA famousPeople arrays are
  // kept in lockstep — so we can use the English name as the avatar key.
  const enFamous = useMemo(
    () => (locale === 'ja' ? getPremiumTypeDataLocalized(data.code, 'en').famousPeople : null),
    [locale, data.code],
  );
  return (
  <Section id="famous">
    ...
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {data.famousPeople.map((p, i) => {
        ...
        const avatar = getFamousAvatar(enFamous?.[i]?.name ?? p.name);
        return (
